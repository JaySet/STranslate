...
/// <summary>
///     <see href="https://github1s.com/openai-translator/openai-translator/blob/main/src/common/traditional-or-simplified.ts#L15" />
/// </summary>
public class TraditionalSimplifiedUtil
{
    public static bool IsSimplifiedChinese(string content)
    {
        return Detect(content).DetectedCharacters == LangType.Simplified;
    }

    public static bool IsTraditionalChinese(string content)
    {
        return Detect(content).DetectedCharacters == LangType.Traditional;
    }

    private static Result Detect(string content)
    {
        var ret = new Result(content.Length, 0, 0, LangType.Unknown, 0);
        for (var i = 0; i < content.Length; i++)
        {
            var c = content[i];
            if (IsCharSimplified(c))
            {
                ret.SimplifiedCharacters++;
            }
            else if (IsCharTraditional(c))
            {
                ret.TraditionalCharacters++;
            }

            if (i != content.Length - 1) continue;
            
            if (ret.SimplifiedCharacters > ret.TraditionalCharacters)
            {
                ret.DetectedCharacters = LangType.Simplified;
                ret.DetectionRate = ret.TraditionalCharacters == 0 ? 1 : ret.SimplifiedCharacters / ret.TraditionalCharacters;
            }
            else if (ret.SimplifiedCharacters < ret.TraditionalCharacters)
            {
                ret.DetectedCharacters = LangType.Traditional;
                ret.DetectionRate = ret.SimplifiedCharacters == 0 ? 1 : ret.TraditionalCharacters / ret.SimplifiedCharacters;
            }
            else
            {
                ret.DetectedCharacters = LangType.Unknown;
                ret.DetectionRate = 0;
            }
            return ret;
        }
    ...
    private static bool IsCharSimplified(char c)
    {
        return SimplifiedArray.Contains(c);
    }

    private static bool IsCharTraditional(char c)
    {
        return TraditionalArray.Contains(c);
    }

    private static readonly char[] SimplifiedArray =
    ...
    private static readonly char[] TraditionalArray =
...
internal class Result(
    int inputLength,
    int simplifiedCharacters,
    int traditionalCharacters,
    LangType detectedCharacters,
    int detectionRate)
{
    public int InputLength { get; set; } = inputLength;
    public int SimplifiedCharacters { get; set; } = simplifiedCharacters;
    public int TraditionalCharacters { get; set; } = traditionalCharacters;
    public LangType DetectedCharacters { get; set; } = detectedCharacters;
    public int DetectionRate { get; set; } = detectionRate;
}